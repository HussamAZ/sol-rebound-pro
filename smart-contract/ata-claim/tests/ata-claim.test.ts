...
// استيراد الأنواع بشكل منفصل باستخدام 'import type' (يبقى كما هو)
...
describe("ata-claim", () => {
    ...
    const payerWallet = (provider.wallet as WalletType).payer; // استخدم WalletType المستورد كنوع
    console.log(`Test Payer Wallet (from Anchor Env): ${payerWallet.publicKey.toBase58()}`);

    const program = anchor.workspace.AtaClaim as ProgramType<AtaClaim>; // استخدم ProgramType المستورد كنوع
    ...
    let userKeypair: Keypair; // المستخدم الذي سيقوم بالإغلاق (سيتم تمويله)

    // --- دالة مساعدة لجلب الرصيد ---
    const getBalance = async (publicKey: PublicKey): Promise<BNType> => {
    ...
    async function ensureFunded(accountPublicKey: PublicKey, minLamports: BNType, payer: Keypair, label: string): Promise<void> { // <-- استخدم BNType هنا
  ...
  describe("close_multiple_atas", () => {
        ...
        let userAta1: PublicKey; // حساب ATA للاختبار
        ...
        beforeEach(async () => {
            ...
            const ataInfo = await getOrCreateAssociatedTokenAccount(
                connection,
                payerWallet,        // Payer for creation
        ...
        it("Closes multiple (2) empty ATAs without a referrer", async () => {
            ...
            const ataInfo2 = await getOrCreateAssociatedTokenAccount(
                connection,
                payerWallet,
                mint2, // <-- استخدام Mint الجديد
            ...
            try { await getAccount(connection, userAta1); } catch (e: any) { expect(e.name).to.equal("TokenAccountNotFoundError"); error1Found = true; }
            try { await getAccount(connection, userAta2); } catch (e: any) { expect(e.name).to.equal("TokenAccountNotFoundError"); error2Found = true; }
        ...
        it("Closes a single empty ATA with a referrer", async () => {
            ...
            const txSignature = await program.methods
                ...
                .remainingAccounts([ // يجب تمرير كلا الحسابين: ATA والمحيل
                    { pubkey: userAta1, isSigner: false, isWritable: true },
            ...
            let errorFound = false;
            try { await getAccount(connection, userAta1); } catch (e: any) { expect(e.name).to.equal("TokenAccountNotFoundError"); errorFound = true; }
        ...
        it("Fails if referrer account is not writable", async () => {
            ...
            await provider.sendAndConfirm( // يجب أن يكون الحساب موجودًا لهذا الاختبار
                 new Transaction().add(SystemProgram.transfer({ fromPubkey: payerWallet.publicKey, toPubkey: referrerKP.publicKey, lamports: LAMPORTS_PER_SOL / 100 })),
            ...
            try {
                ...
                if (error instanceof anchor.AnchorError) {
                     ...
                     expect(isConstraintError || isCustomError,
                          `Expected ConstraintMut or ReferrerAccountNotWritable, but got ${error.error.errorCode.code}`
                     ).to.be.true;
                }
            }
        });
    ...
    describe("distribute_rewards", () => {
        ...
        beforeEach(async () => {
            ...
            await provider.sendAndConfirm(
                 new Transaction().add(SystemProgram.transfer({ fromPubkey: payerWallet.publicKey, toPubkey: recipient1.publicKey, lamports: LAMPORTS_PER_SOL / 100 })),
                 [payerWallet], { commitment: "confirmed", skipPreflight: true }
            );
            console.log(`Funding recipient2 ${recipient2.publicKey.toBase58()}...`);
            await provider.sendAndConfirm(
                 new Transaction().add(SystemProgram.transfer({ fromPubkey: payerWallet.publicKey, toPubkey: recipient2.publicKey, lamports: LAMPORTS_PER_SOL / 100 })),
                 [payerWallet], { commitment: "confirmed", skipPreflight: true }
            );
        ...
        it("Successfully distributes rewards from treasury by authority", async () => {
            ...
            const txSignature = await web3.sendAndConfirmTransaction( // استخدام web3 مباشرة
                connection,
                transaction,
                [authorityKeypair, treasury], // <-- تمرير كلا الموقعين هنا!
        ...
        it("Fails to distribute rewards if signer is not the authority", async () => {
            ...
            await provider.sendAndConfirm(
                 new Transaction().add(SystemProgram.transfer({ fromPubkey: payerWallet.publicKey, toPubkey: wrongSigner.publicKey, lamports: LAMPORTS_PER_SOL / 10 })),
                 [payerWallet], { commitment: "confirmed", skipPreflight: true }
            );
            ...
            try {
                ...
                await web3.sendAndConfirmTransaction(
                    connection,
                    transaction,
                    [wrongSigner, treasury], // <-- الموقع الخاطئ و treasury
        ...
        it("Fails to distribute rewards if treasury balance is insufficient", async () => {
            ...
            try {
                ...
                await web3.sendAndConfirmTransaction(
                    connection,
                    transaction,
                    [authorityKeypair, treasury],
                    { commitment: "confirmed", skipPreflight: true } // **هام: skipPreflight=true** قد يكون ضروريًا هنا لرؤية خطأ البرنامج الفعلي بدلاً من فشل المحاكاة
                );
        ...
        it("Fails if recipient list is empty", async () => {
            const amounts: BNType[] = []; // مصفوفة مبالغ فارغة
